...
local function switch_impl_intf(bufnr, client)
    ...
    if not uri then return vim.notify("could not get URI for current buffer") end
    local params = { uri }
    ---@diagnostic disable-next-line:param-type-mismatch
    client:request(method_name, params, function(err, result)
        ...
        elseif #result == 1 then
            ...
            vim.ui.select(
                ...
                function(choice)
                    if choice then vim.cmd.edit(vim.uri_to_fname(choice)) end
                end
            )
        end
    end, bufnr)
...
return {
    ...
    on_attach = function(client, bufnr)
        vim.api.nvim_buf_create_user_command(
            ...
            function() switch_impl_intf(bufnr, client) end,
